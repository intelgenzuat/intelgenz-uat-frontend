...
import { mitigationttpview, View, Viewreport, IntelCard, IntelCardDetails, ThreatActorprofile, IntelCardMalware, IntelCardThreatDetails, BlogsandNews, blogsnewslist } from '../Routes/Routes';
...
export default function Layout({ noFooter, showAdminSidebar }) {
  ...
  const hideFooterRoutes = [View, Viewreport, mitigationttpview, IntelCard, IntelCardDetails, ThreatActorprofile, IntelCardMalware, IntelCardThreatDetails];
  ...
  return (
    <div className={`d-flex flex-column ${shouldHideFooter || isScrollableInner ? 'vh-100 overflow-hidden' : 'min-vh-100'} bg-body`}>
      <Header toggleSidebar={toggleSidebar} showHamburger={showAdminSidebar || location.pathname === View || location.pathname === mitigationttpview || location.pathname === IntelCard || location.pathname === IntelCardMalware || location.pathname === ThreatActorprofile} />
